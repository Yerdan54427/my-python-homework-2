...
def clear_admission_ticket_files(folder_name):
    # 如果目录不存在，说明没有历史文件需要清理，直接返回
    if not os.path.isdir(folder_name):
        return

    # 只删除目录中已有的 .txt 文件，避免误删其他资源文件
...
4. `file_name = f"{seat_number:02d}.txt"` 让输出文件按 `01.txt`、`02.txt` 这种格式编号，便于人工检查，也便于按文件名顺序查看。
